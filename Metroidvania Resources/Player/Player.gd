...
const DustEffect = preload("res://Metroidvania Resources/Effects/DustEffect.tscn")
const PlayerBullet = preload("res://Metroidvania Resources/Player/PlayerBullet.tscn")
const JumpEffect = preload("res://Metroidvania Resources/Effects/JumpEffect.tscn")
const WallDustEffect = preload("res://Metroidvania Resources/Effects/WallSlideEffect.tscn")
const PlayerMissile = preload("res://Metroidvania Resources/Player/PlayerMissile.tscn")
...
export(int) var MAX_SLOPE_ANGLE = 46 # 45 seems to not quite do it
...
onready var muzzle = $Sprite/PlayerGun/Sprite/Muzzel
...
func jump(force):
	SoundFx.play("Jump", rand_range(0.8, 1.1), -15)
...
func apply_gravity(delta):
	if not is_on_floor():
		...
		motion.y = min(motion.y, JUMP_FORCE) # so that we never fall faster then the JUMP_FORCE
...
func move():
	...
	var was_on_floor = is_on_floor() # need to do a check before we move
	var last_motion = motion
	var last_position = position
	
	motion = move_and_slide_with_snap(motion, snap_vector*4, Vector2.UP, \
			true, 4, deg2rad(MAX_SLOPE_ANGLE)) # snaps character to ground
	...
	# Prevent Sliding (hack) - STILL A BUG
