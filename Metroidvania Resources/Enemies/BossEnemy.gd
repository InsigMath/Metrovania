...
const Bullet = preload("res://Metroidvania Resources/Player/EnemyBullet.tscn")
...
func chase_player(delta):
	...
	if player != null:
		var direction_to_move = sign(player.global_position.x - \
		global_position.x)
		motion.x +=  ACCELERATION * delta * direction_to_move
		motion.x = clamp(motion.x, -MAX_SPEED, MAX_SPEED)
		global_position.x += motion.x * delta
		rotation_degrees = lerp(rotation_degrees, (motion.x / MAX_SPEED) * \
		10, 0.3)
